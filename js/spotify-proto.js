...
if (url.includes("bootstrap/v1/bootstrap") && method === postMethod) {
  ...
  $notification.post(
    "bootstrap - Spotify Unlock Premium",
    JSON.stringify(bootstrapResponseObj, null, 2)
  );
} else if (
  ...
  $notification.post(
    "customize - Spotify Unlock Premium",
    JSON.stringify(ucsResponseWrapperMessage, null, 2)
  );
} else {
  $notification.post(
    "Spotify Unlock Premium",
    "Path/Request method match error:",
    method + "," + url
  );
}
if (isQuanX) {
...
function processMapObj(accountAttributesMapObj) {
  ...
  // Membership ad tab in the lower right corner of the home page
  ...
  accountAttributesMapObj["product"] = { stringValue: "premium" };
  ...
  // Determines whether customization is valid. Some users do not have this attribute
    ...
    if (resStatus !== 200) {
        ...
        if (url.includes("bootstrap/v1/bootstrap") && method === postMethod) {
            ...
            $notification.post(
                "bootstrap",
                "Spotify Unlock Premium",
                JSON.stringify(bootstrapResponseObj, null, 2) // Chuyển đổi thành JSON có định dạng đẹp
            );
        } else if (
            url.includes("user-customization-service/v1/customize") &&
            method === postMethod
...
>>>>>>> eff9a5b2311a1499f23a7fbfc35a3f81390b0644
